...
public class TempMute extends CommandTemplate {
    public TempMute(Eclipse plugin) {
        super(plugin, "tempmute", List.of("tmute"), EclipsePermissionManager.EclipsePerm.MODERATION_TEMPMUTE, false, 0, "Temporarily mute a player");
    }
    ...
    protected boolean perform(CommandSender sender, Player player, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(plugin.getMessage("tempMuteUsage"));
            return true;
        }
        ...
        Punishment p = mm.punish(PunishmentType.TEMP_MUTE, targetUUID, targetName, issuerUUID, issuerName, reason, duration, silent);
        if (p != null) {
            mm.broadcastPunishment(p, targetName);
            Player target = Bukkit.getPlayer(targetUUID);
            if (target != null && target.isOnline()) {
                target.sendMessage(mm.buildMuteMessage(p));
            }
        }
        return true;
    }
