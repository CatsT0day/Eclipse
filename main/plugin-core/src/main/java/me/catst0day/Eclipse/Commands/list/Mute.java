...
public class Mute extends CommandTemplate {
    public Mute(Eclipse plugin) {
        super(plugin, "mute", List.of(), EclipsePermissionManager.EclipsePerm.MODERATION_MUTE, false, 0, "Permanently mute a player");
    }
    ...
    protected boolean perform(CommandSender sender, Player player, String[] args) {
        ...
        if (args.length < 1) {
            sender.sendMessage(plugin.getMessage("muteUsage"));
            return true;
        }
        ...
        Punishment p = mm.punish(PunishmentType.MUTE, targetUUID, targetName, issuerUUID, issuerName, reason, 0, silent);
        if (p != null) {
